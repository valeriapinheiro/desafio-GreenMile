...
- Problema identificado: Durante a elaboração dos teste com o cypress, não foi possível identificar algum comando no cypress para acionar o evento de long click realizado pela biblioteca sugerida. Primeira possibilidade seria simular este evento sem o conhecimento interno do código, realizando o teste apenas na interface (não consegui realizar essa alternativa). Segunda possibilidade, com o acesso ao código e liberdade para altera-lo, foi realizado a implementação do evento sem a utilização da biblioteca hammer.js, utilizando so JavaScript, dessa forma foi possível validar o tamanho do componente após o long click diretamente. 
